...
def generatelocation(locations,name):
	t = open(name+'.locations.json','w')
	content = ' "{}" :{{\
	\n   "coordinates": {},\
	\n   "neighbours": {},\
	\n   "hosts":{},\
	...
	for k in locations:
		if count > lens - 2:
			...
			count = count+1
		contentr = content.format(int(k[0])-453,k[1],k[2],k[3],k[4],k[5],k[6])
		t.write(contentr)
	content = ' "{}" :{{\
	\n   "coordinates": {},\
	\n   "neighbours": {},\
...
def getcap():
	ids  = []
	mos = []
	with open('../file/mosBreedingSiteClassification.csv') as csvfile:
		reader = csv.DictReader(csvfile)
		for row in reader:
...
def generate_location(name,location_config):
	...
	for rowno in range(1,310):
		...
		# if caps  = 1 -> capbility = 20, this can be adjusted
			if caps == '1':
				capss = location_config['1']
			elif caps == '2':
				capss = location_config['2']
			else:
				capss = location_config['other'] # to adjust

			par = [cellID,[float(longs[colno-1]),float(lats[rowno-1])],generateneighbour(rowno,colno),[],capss,0,[capss,0,0,0]]
			loca.append(par)
			# print(rowno,colno)
	generatelocation(loca,name)
